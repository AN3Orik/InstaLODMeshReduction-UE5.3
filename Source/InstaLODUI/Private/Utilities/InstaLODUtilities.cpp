...
namespace InstaLODVectorHelper
{
	...
	{
		TVertexAttributesRef<FVector3f> VertexPositions = OutRawMesh.VertexAttributes().GetAttributesRef<FVector3f>(
		...
		TVertexInstanceAttributesRef<FVector3f> VertexInstanceNormals = OutRawMesh.VertexInstanceAttributes().
			GetAttributesRef<FVector3f>(MeshAttribute::VertexInstance::Normal);
		TVertexInstanceAttributesRef<FVector3f> VertexInstanceTangents = OutRawMesh.VertexInstanceAttributes().
			GetAttributesRef<FVector3f>(MeshAttribute::VertexInstance::Tangent);
		...
		TVertexInstanceAttributesRef<FVector4f> VertexInstanceColors = OutRawMesh.VertexInstanceAttributes().
			GetAttributesRef<FVector4f>(MeshAttribute::VertexInstance::Color);
		TVertexInstanceAttributesRef<FVector2f> VertexInstanceUVs = OutRawMesh.VertexInstanceAttributes().
		...
		{
			FVector3f TangentY = FVector3f::CrossProduct(VertexInstanceNormals[VertexInstanceID],
			...
			VertexInstanceBinormalSigns[VertexInstanceID] = GetBasisDeterminantSign(
				FVector3d(VertexInstanceTangents[VertexInstanceID]), FVector3d(TangentY),
				FVector3d(VertexInstanceNormals[VertexInstanceID]));
		}
...
{
	public:
		UMaterialOptions* PopulateMaterialOptions(
			const FMaterialProxySettings& MaterialSettings) const
		{
			UMaterialOptions* MaterialOptions = DuplicateObject(GetMutableDefault<UMaterialOptions>(),
			                                                    GetTransientPackage());
		...
		void PopulatePropertyEntry(const FMaterialProxySettings& MaterialSettings,
		                                                EMaterialProperty MaterialProperty,
		                                                FPropertyEntry& InOutPropertyEntry) const
		{
			...
			switch (MaterialSettings.TextureSizingType)
			{
				...
				case TextureSizingType_UseManualOverrideTextureSize:
					{
						...
						InOutPropertyEntry.CustomSize = [MaterialSettings, MaterialProperty]() -> FIntPoint
						{
							...
							{
								case MP_BaseColor:
									return MaterialSettings.DiffuseTextureSize;
								case MP_Specular:
									return MaterialSettings.SpecularTextureSize;
								case MP_Roughness:
									return MaterialSettings.RoughnessTextureSize;
								case MP_Anisotropy:
									return MaterialSettings.AnisotropyTextureSize;
								case MP_Metallic:
									return MaterialSettings.MetallicTextureSize;
								case MP_Normal:
									return MaterialSettings.NormalTextureSize;
								case MP_Tangent:
									return MaterialSettings.TangentTextureSize;
								case MP_Opacity:
									return MaterialSettings.OpacityTextureSize;
								case MP_OpacityMask:
									return MaterialSettings.OpacityMaskTextureSize;
								case MP_EmissiveColor:
									return MaterialSettings.EmissiveTextureSize;
								case MP_AmbientOcclusion:
									return MaterialSettings.AmbientOcclusionTextureSize;
								default:
									{
				...
				case TextureSizingType_UseAutomaticBiasedSizes:
					{
						...
						const FIntPoint HalfRes = FIntPoint(FMath::Max(8, FullRes.X >> 1),
						                                    FMath::Max(8, FullRes.Y >> 1));
						const FIntPoint QuarterRes = FIntPoint(FMath::Max(4, FullRes.X >> 2),
						                                       FMath::Max(4, FullRes.Y >> 2));

						InOutPropertyEntry.bUseCustomSize = true;
						InOutPropertyEntry.CustomSize = [FullRes, HalfRes, QuarterRes, MaterialSettings,
								MaterialProperty]() -> FIntPoint
							{
								switch (MaterialProperty)
								{
									case MP_Normal:
				...
				default:
					UE_LOG(LogInstaLOD, Error,
					       TEXT(
						       "Unsupported TextureSizingType value. You should resolve the material texture size first with ResolveTextureSize()"
					       ));
			}
			...
			InOutPropertyEntry.bUseConstantValue = [MaterialSettings, MaterialProperty]() -> bool
			{
				switch (MaterialProperty)
				{
					case MP_BaseColor:
						return false;
					case MP_Normal:
						return !MaterialSettings.bNormalMap;
					case MP_Tangent:
						return !MaterialSettings.bTangentMap;
					case MP_Specular:
						return !MaterialSettings.bSpecularMap;
					case MP_Roughness:
						return !MaterialSettings.bRoughnessMap;
					case MP_Anisotropy:
						return !MaterialSettings.bAnisotropyMap;
					case MP_Metallic:
						return !MaterialSettings.bMetallicMap;
					case MP_Opacity:
						return !MaterialSettings.bOpacityMap;
					case MP_OpacityMask:
						return !MaterialSettings.bOpacityMaskMap;
					case MP_EmissiveColor:
						return !MaterialSettings.bEmissiveMap;
					case MP_AmbientOcclusion:
						return !MaterialSettings.bAmbientOcclusionMap;
					default:
						{
							checkf(false, TEXT("Invalid Material Property"));
							return false;
			...
			InOutPropertyEntry.ConstantValue = [MaterialSettings, MaterialProperty]() -> float
			{
				switch (MaterialProperty)
				{
					...
					case MP_Tangent:
						return 1.0f;
					case MP_Specular:
						return MaterialSettings.SpecularConstant;
					case MP_Roughness:
						return MaterialSettings.RoughnessConstant;
					case MP_Anisotropy:
						return MaterialSettings.AnisotropyConstant;
					case MP_Metallic:
						return MaterialSettings.MetallicConstant;
					case MP_Opacity:
						return MaterialSettings.OpacityConstant;
					case MP_OpacityMask:
						return MaterialSettings.OpacityMaskConstant;
					case MP_EmissiveColor:
						return 0.0f;
					case MP_AmbientOcclusion:
						return MaterialSettings.AmbientOcclusionConstant;
					default:
						{
							checkf(false, TEXT("Invalid Material Property"));
							return 1.0f;
		...
		EFlattenMaterialProperties ToFlattenProperty(EMaterialProperty MaterialProperty) const
		{
			switch (MaterialProperty)
			{
				case EMaterialProperty::MP_BaseColor:
					return EFlattenMaterialProperties::Diffuse;
				case EMaterialProperty::MP_Metallic:
					return EFlattenMaterialProperties::Metallic;
				case EMaterialProperty::MP_Specular:
					return EFlattenMaterialProperties::Specular;
				case EMaterialProperty::MP_Roughness:
					return EFlattenMaterialProperties::Roughness;
				case EMaterialProperty::MP_Anisotropy:
					return EFlattenMaterialProperties::Anisotropy;
				case EMaterialProperty::MP_Normal:
					return EFlattenMaterialProperties::Normal;
				case EMaterialProperty::MP_Tangent:
					return EFlattenMaterialProperties::Tangent;
				case EMaterialProperty::MP_Opacity:
					return EFlattenMaterialProperties::Opacity;
				case EMaterialProperty::MP_EmissiveColor:
					return EFlattenMaterialProperties::Emissive;
				case EMaterialProperty::MP_OpacityMask:
					return EFlattenMaterialProperties::OpacityMask;
				case EMaterialProperty::MP_AmbientOcclusion:
					return EFlattenMaterialProperties::AmbientOcclusion;
				default:
					return EFlattenMaterialProperties::NumFlattenMaterialProperties;
			}
		}

		void TransferOutputToFlatMaterials(const TArray<FMaterialData>& InMaterialData,
		                                   TArray<FBakeOutput>& InOutBakeOutputs,
		...
		void CreateMaterialData(IInstaLOD* InstaLOD, TArray<InstaLODMergeData>& MergeData,
		                        InstaLOD::IInstaLODMaterialData* MaterialData,
		                        const struct FMaterialProxySettings& InMaterialProxySettings,
		                        TArray<UMaterialInterface*>& OutMaterials) const
		{
			check(InstaLOD);
			check(MaterialData);
			...
			SlowTask.EnterProgressFrame(10.0f, NSLOCTEXT("InstaLODUI", "CreateMaterialData_GatheringData",
			                                             "Gathering Data"));
			SlowTask.MakeDialog();

			IMaterialBakingModule& MaterialBakingModule = FModuleManager::Get().LoadModuleChecked<
				IMaterialBakingModule>("MaterialBaking");
			...
			const auto fnScaleTextureCoordinatesToBox = [](const FBox2D& Box,
			                                               TArray<FVector2D>& InOutTextureCoordinates)
			{
			...
			for (const InstaLODMergeData& InstaLODMergeData : MergeData)
			{
				...
				{
					FMeshMergeHelpers::ExtractSections(InstaLODMergeData.Component->StaticMeshComponent.Get(),
					                                   BaseLODIndex, Sections);
				}
				else if (InstaLODMergeData.Component->SkeletalMeshComponent.IsValid())
				{
					FMeshMergeHelpers::ExtractSections(InstaLODMergeData.Component->SkeletalMeshComponent.Get(),
					                                   BaseLODIndex, Sections);
					IInstaLOD::UE_SkeletalMeshResource* const Resource = InstaLODMergeData.Component->
						SkeletalMeshComponent->GetSkeletalMeshAsset()->GetImportedModel();
				...
				InstaLOD->ConvertInstaLODMeshToMeshDescription(InstaLODMergeData.InstaLODMesh, TMap<int32, FName>(),
				                                               MeshDescription);
				...
				for (const FSectionInfo& Section : Sections)
				{
					...
					FMaterialUtilities::AnalyzeMaterial(Section.Material, MaterialProperties, NumTexCoords,
					                                    bUseVertexData);
				...
				for (const bool bRequiresFullBake : MaterialRequiresFullBake)
				{
					...
					const UStaticMeshComponent* const StaticMeshComponent = InstaLODMergeData.Component->
						StaticMeshComponent.Get();
					const UStaticMesh* const StaticMesh = StaticMeshComponent != nullptr
					...
					if (StaticMesh != nullptr && (StaticMesh->GetSourceModel(0).BuildSettings.bGenerateLightmapUVs ||
						(StaticMesh->GetLightMapCoordinateIndex() != 0 && RawMesh.WedgeTexCoords[StaticMesh->
							GetLightMapCoordinateIndex()].Num() != 0)))
					{
						MeshSetting.CustomTextureCoordinates = InstaLODVectorHelper::FVector2fArrayToFVector2dArray(
							RawMesh.WedgeTexCoords[StaticMeshComponent->GetStaticMesh()->GetLightMapCoordinateIndex()]);

						fnScaleTextureCoordinatesToBox(FBox2D(FVector2D::ZeroVector, FVector2D(1, 1)),
						                               MeshSetting.CustomTextureCoordinates);
					}
					else
					{
						// generate new non overlapping UVs
						IMeshUtilities& MeshUtilities = FModuleManager::GetModuleChecked<IMeshUtilities>(
							"MeshUtilities");
					...
					FStaticMeshOperations::ConvertFromRawMesh(RawMesh, MeshDescription, /*MaterialMap*/
					                                          TMap<int32, FName>(), /*bSkipNormalsAndTangents*/ false);
					...
					{
						if (const FMeshMapBuildData* const MeshMapBuildData = StaticMeshComponent->GetMeshMapBuildData(
							*StaticMeshComponent->LODData.GetData()))
						{
							MeshSetting.LightMap = MeshMapBuildData->LightMap;
							MeshSetting.LightMapIndex = StaticMeshComponent->GetStaticMesh()->
				...
				// progress state 
				SlowTask.EnterProgressFrame(10.0f, NSLOCTEXT("InstaLODUI", "CreateMaterialData_FlatteningMaterialData",
				                                             "Flattening Material Data"));
				SlowTask.MakeDialog();
				...
				for (const FSectionInfo& Section : Sections)
				{
					...
					for (const FPropertyEntry& Entry : Options->Properties)
					{
						if (!Entry.bUseConstantValue && MaterialSetting.Material->IsPropertyActive(Entry.Property) &&
							Entry.Property != MP_MAX)
						{
							MaterialSetting.PropertySizes.Add(Entry.Property,
							                                  Entry.bUseCustomSize
								                                  ? Entry.CustomSize
								                                  : Options->TextureSize);
						}
					}
					...
					MaterialBakingModule.BakeMaterials(TArray<FMaterialData*>({&MaterialSetting}),
					                                   TArray<FMeshData*>({&MeshSetting}), TempBakeOutputs);
					BakeOutputs.Append(TempBakeOutputs);

					// progress state 
					const float Progress = (SectionIndex + 1) / (float)NumSections;
					SlowTask.EnterProgressFrame(Progress, FText::FromString(
						                            FString::Printf(
							                            TEXT("Flattening Material (%i/%i)"), SectionIndex + 1,
							                            NumSections)));
					SlowTask.MakeDialog();
				...
				{
					NewUVs = MeshSetting.CustomTextureCoordinates.Num()
						         ? InstaLODVectorHelper::FVector2dArrayToFVector2fArray(
							         MeshSetting.CustomTextureCoordinates)
						         : RawMesh.WedgeTexCoords[MeshSetting.TextureCoordinateIndex];
				}
				...
				{
					uint64 WedgeCount = 0;
					InstaLOD::InstaVec2F* const InstaLODWedgeTexCoords0 = InstaLODMesh->GetWedgeTexCoords(
						0, &WedgeCount);
			...
			for (const FPropertyEntry& Entry : Options->Properties)
			{
				if (Entry.bUseConstantValue && Entry.Property != MP_MAX)
				{
					...
					ConstantData[0] = FLinearColor(Entry.ConstantValue, Entry.ConstantValue, Entry.ConstantValue).
						ToFColor(true);
			...
			const IMeshMergeUtilities& Module = FModuleManager::Get().LoadModuleChecked<IMeshMergeModule>(
				"MeshMergeUtilities").GetUtilities();
			((FInstaLODMeshMergeUtilities*)&Module)->TransferOutputToFlatMaterials(MaterialSettings, BakeOutputs, FlattenedMaterials);
			InstaLOD->ConvertFlattenMaterialsToInstaLODMaterialData(FlattenedMaterials, MaterialData,
			                                                        InMaterialProxySettings);
		}
};

void UInstaLODUtilities::CreateMaterialData(class IInstaLOD* InstaLOD, TArray<InstaLODMergeData>& MergeData,
                                            class InstaLOD::IInstaLODMaterialData* MaterialData,
                                            const struct FMaterialProxySettings& InMaterialProxySettings,
                                            TArray<UMaterialInterface*>& OutMaterials)
{
	...
	const IMeshMergeUtilities& Module = FModuleManager::Get().LoadModuleChecked<IMeshMergeModule>("MeshMergeUtilities").
	                                                          GetUtilities();
	((FInstaLODMeshMergeUtilities*)&Module)->CreateMaterialData(InstaLOD, MergeData, MaterialData,
	                                                            InMaterialProxySettings, OutMaterials);
}
...
void UInstaLODUtilities::AppendMeshComponentToInstaLODMesh(IInstaLOD* InstaLOD,
                                                           TSharedPtr<FInstaLODMeshComponent> MeshComponent,
                                                           InstaLOD::IInstaLODMesh* OutInstaLODMesh, int32 BaseLODIndex)
{
	check(InstaLOD);
	...
	InstaLOD::IInstaLODMeshExtended* const ExtendedMesh = InstaLOD->GetInstaLOD()->CastToInstaLODMeshExtended(
		OutInstaLODMesh);
...
namespace FProxyMaterialUtilities_Copy
{
	 ...
	 * Find the parameter name to use with the provided material, for the given property. 
	 */
	static bool GetMatchingParamName(EFlattenMaterialProperties InProperty, const UMaterialInterface* InBaseMaterial,
	                                 FString& OutParamName, TArray<FString>* OutMissingNames = nullptr)
	{
		...
		for (const FString& PotentialName : PotentialNames)
		{
			...
			switch (GetConstantParamType(InProperty))
			{
				case EMaterialParameterType::Scalar:
					...
					bHasRequiredParams &= InBaseMaterial->GetStaticSwitchParameterDefaultValue(
							UseTexture, DefaultSwitchValue, ExpressionGuid) &&
	...
	static FString GetMatchingParamName(EFlattenMaterialProperties InProperty, UMaterialInterface* InBaseMaterial)
	{
		...
		{
			UE_LOG(LogInstaLOD, Fatal,
			       TEXT("Invalid base material, should have been rejected by IsValidFlattenMaterial()"));
		}
		return ParamName;
	}

	static bool CalculatePackedTextureData(const FFlattenMaterial& InMaterial, bool& bOutPackMetallic,
	                                       bool& bOutPackSpecular, bool& bOutPackRoughness, int32& OutNumSamples,
	                                       FIntPoint& OutSize)
	{
		// Whether or not a material property is baked down
		const bool bHasMetallic = InMaterial.DoesPropertyContainData(EFlattenMaterialProperties::Metallic) && !
			InMaterial.IsPropertyConstant(EFlattenMaterialProperties::Metallic);
		const bool bHasRoughness = InMaterial.DoesPropertyContainData(EFlattenMaterialProperties::Roughness) && !
			InMaterial.IsPropertyConstant(EFlattenMaterialProperties::Roughness);
		const bool bHasSpecular = InMaterial.DoesPropertyContainData(EFlattenMaterialProperties::Specular) && !
			InMaterial.IsPropertyConstant(EFlattenMaterialProperties::Specular);
		...
		if (OutNumSamples != 0)
		{
			bOutPackMetallic = bHasMetallic
				                   ? (OutNumSamples == InMaterial.GetPropertySamples(
					                   EFlattenMaterialProperties::Metallic).Num())
				                   : false;
			NumPacked += (bOutPackMetallic) ? 1 : 0;
			bOutPackRoughness = bHasRoughness
				                    ? (OutNumSamples == InMaterial.GetPropertySamples(
					                    EFlattenMaterialProperties::Roughness).Num())
				                    : false;
			NumPacked += (bOutPackRoughness) ? 1 : 0;
			bOutPackSpecular = bHasSpecular
				                   ? (OutNumSamples == InMaterial.GetPropertySamples(
					                   EFlattenMaterialProperties::Specular).Num())
				                   : false;
...
UMaterialInstanceConstant* UInstaLODUtilities::CreateFlattenMaterialInstance(
	const FFlattenMaterial& FlattenMaterial, const FMaterialProxySettings& InMaterialProxySettings,
	const FString& SaveObjectPath, TArray<UObject*>& OutAssetsToSync, bool bIsFlipbookMaterial)
{
	...
	const FString BaseMaterialAsset = bIsFlipbookMaterial
		                                  ? TEXT(
			                                  "/InstaLODMeshReduction/InstaLODFlattenFlipbookMaterial.InstaLODFlattenFlipbookMaterial")
		                                  : TEXT(
			                                  "/InstaLODMeshReduction/InstaLODFlattenMaterial.InstaLODFlattenMaterial");

	UMaterial* const BaseMaterial = LoadObject<UMaterial>(nullptr, *BaseMaterialAsset, nullptr, LOAD_None, nullptr);
	check(BaseMaterial);

	UMaterialInstanceConstant* const OutMaterial = FMaterialUtilities::CreateInstancedMaterial(
		BaseMaterial, nullptr, AssetBasePath + AssetBaseName, RF_Public | RF_Standalone);
	...
	OutMaterial->BasePropertyOverrides.bOverride_DitheredLODTransition = FlattenMaterial.bDitheredLODTransition !=
		false;
	...
	const bool bPackTextures = FProxyMaterialUtilities_Copy::CalculatePackedTextureData(
		FlattenMaterial, bPackMetallic, bPackSpecular, bPackRoughness, NumSamples, PackedSize);
	...
	const auto CreateTextureFromDefault = [&](const FName TextureName, const FString& AssetLongName, FIntPoint Size,
	                                          const TArray<FColor>& Samples,
	                                          const TextureCompressionSettings Compression, const bool bSRGB)
	{
		...
		return FMaterialUtilities::CreateTexture(InOuter, AssetLongName, Size, Samples, CreateParams,
		                                         RF_Public | RF_Standalone);
	};

	const auto SetTextureParam = [&](EFlattenMaterialProperties FlattenProperty,
	                                 const TextureCompressionSettings Compression, const bool bSRGB)
	{
		if (FlattenMaterial.DoesPropertyContainData(FlattenProperty) && !FlattenMaterial.IsPropertyConstant(
			FlattenProperty))
		{
			const FString PropertyName = FProxyMaterialUtilities_Copy::GetMatchingParamName(
				FlattenProperty, OutMaterial);
			const FName TextureName(PropertyName + TEXT("Texture"));
			const FName UseTexture(TEXT("Use") + PropertyName);
			UTexture2D* const Texture = CreateTextureFromDefault(TextureName,
			                                                     AssetBasePath + TEXT("T_") + AssetBaseName + TEXT("_")
			                                                     + PropertyName,
			                                                     FlattenMaterial.GetPropertySize(FlattenProperty),
			                                                     FlattenMaterial.GetPropertySamples(FlattenProperty),
			                                                     Compression, bSRGB);
	...
	const auto SetTextureParamConstVector = [&](EFlattenMaterialProperties FlattenProperty,
	                                            const TextureCompressionSettings Compression, const bool bSRGB)
	{
		if (FlattenMaterial.DoesPropertyContainData(FlattenProperty) && !FlattenMaterial.IsPropertyConstant(
			FlattenProperty))
		...
		else
		{
			...
			OutMaterial->SetVectorParameterValueEditorOnly(
				ConstName, FlattenMaterial.GetPropertySamples(FlattenProperty)[0]);
		}
	};

	const auto SetTextureParamConstScalar = [&](EFlattenMaterialProperties FlattenProperty, float ConstantValue,
	                                            const TextureCompressionSettings Compression, const bool bSRGB)
	{
		if (FlattenMaterial.DoesPropertyContainData(FlattenProperty) && !FlattenMaterial.IsPropertyConstant(
			FlattenProperty))
		...
		else
		{
			const FString PropertyName = FProxyMaterialUtilities_Copy::GetMatchingParamName(
				FlattenProperty, OutMaterial);
			const FName ConstName(PropertyName + TEXT("Const"));
			const FLinearColor Colour = FlattenMaterial.IsPropertyConstant(FlattenProperty)
				                            ? FLinearColor::FromSRGBColor(
					                            FlattenMaterial.GetPropertySamples(FlattenProperty)[0])
				                            : FLinearColor(ConstantValue, 0, 0, 0);
			OutMaterial->SetScalarParameterValueEditorOnly(ConstName, Colour.R);
		}
	};

	const auto SetTextureParamConstLinear = [&](EFlattenMaterialProperties FlattenProperty, float ConstantValue,
	                                            const TextureCompressionSettings Compression, const bool bSRGB)
	{
		if (FlattenMaterial.DoesPropertyContainData(FlattenProperty) && !FlattenMaterial.IsPropertyConstant(
			FlattenProperty))
		...
		else
		{
			...
			const FName ConstName(PropertyName + TEXT("Const"));
			OutMaterial->SetVectorParameterValueEditorOnly(
				ConstName, FlattenMaterial.GetPropertySamples(FlattenProperty)[0].ReinterpretAsLinear());
		}
	};
	...
	if (!bPackMetallic && (FlattenMaterial.GetPropertySize(EFlattenMaterialProperties::Metallic).Num() > 0 || !
		InMaterialProxySettings.bMetallicMap))
	{
		SetTextureParamConstScalar(EFlattenMaterialProperties::Metallic, 0.0f, TC_Default, /*bSRGB:*/ true);
	}

	if (!bPackRoughness && (FlattenMaterial.GetPropertySize(EFlattenMaterialProperties::Roughness).Num() > 0 || !
		InMaterialProxySettings.bRoughnessMap))
	{
		SetTextureParamConstScalar(EFlattenMaterialProperties::Roughness, 0.0f, TC_Default, /*bSRGB:*/ true);
	}

	if (!bPackSpecular && (FlattenMaterial.GetPropertySize(EFlattenMaterialProperties::Specular).Num() > 0 || !
		InMaterialProxySettings.bSpecularMap))
	{
		SetTextureParamConstScalar(EFlattenMaterialProperties::Specular, 0.0f, TC_Default, /*bSRGB:*/ true);
	}

	if (FlattenMaterial.GetPropertySize(EFlattenMaterialProperties::Opacity).Num() > 0 || !InMaterialProxySettings.
		bOpacityMap)
	{
		SetTextureParamConstScalar(EFlattenMaterialProperties::Opacity, 1.0f, TC_Grayscale, /*bSRGB:*/ false);
	}

	if (FlattenMaterial.GetPropertySize(EFlattenMaterialProperties::OpacityMask).Num() > 0 || !InMaterialProxySettings.
		bOpacityMaskMap)
	{
		SetTextureParamConstScalar(EFlattenMaterialProperties::OpacityMask, 1.0f, TC_Grayscale, /*bSRGB:*/ false);
	}

	if (FlattenMaterial.GetPropertySize(EFlattenMaterialProperties::AmbientOcclusion).Num() > 0 || !
		InMaterialProxySettings.bAmbientOcclusionMap)
	{
	...
	if (bPackTextures)
	{
		...
		const EFlattenMaterialProperties Properties[3] = {
			EFlattenMaterialProperties::Metallic, EFlattenMaterialProperties::Roughness,
		...
		for (int32 PropertyIndex = 0; PropertyIndex < 3; ++PropertyIndex)
		{
			...
			const bool HasProperty = PropertyShouldBePack[PropertyIndex] && FlattenMaterial.
				DoesPropertyContainData(Property) && !FlattenMaterial.IsPropertyConstant(Property);
			...
			if (HasProperty)
			{
				...
				for (int32 SampleIndex = 0; SampleIndex < NumSamples; ++SampleIndex)
				{
					...
					MergedTexture[SampleIndex].DWColor() |= (FColor::Black.DWColor() + ((PropertySamples[SampleIndex].
						DWColor() & ColorMask) >> Shift[PropertyIndex]));
				}
			}
		}

		// create texture using the merged property data
		UTexture2D* const PackedTexture = FMaterialUtilities::CreateTexture(
			InOuter, AssetBasePath + TEXT("T_") + AssetBaseName + TEXT("_MRS"),
		...
		OutMaterial->SetStaticSwitchParameterValueEditorOnly(FMaterialParameterInfo(TEXT("PackMetallic")),
		                                                     bPackMetallic);
		OutMaterial->SetStaticSwitchParameterValueEditorOnly(FMaterialParameterInfo(TEXT("PackSpecular")),
		                                                     bPackSpecular);
		OutMaterial->SetStaticSwitchParameterValueEditorOnly(FMaterialParameterInfo(TEXT("PackRoughness")),
		                                                     bPackRoughness);
...
UMaterialInstanceConstant* UInstaLODUtilities::CreateFlattenMaterialInstanceFromInstaMaterial(
	InstaLOD::IInstaLODMaterial* const Material, const FMaterialProxySettings& InMaterialProxySettings,
	const FString& SaveObjectPath, TArray<UObject*>& OutAssetsToSync, bool bIsFlipbookMaterial)
{
	...
	InstaLODModule.GetInstaLODInterface()->ConvertInstaLODMaterialToFlattenMaterial(
		Material, FlattenMaterial, InMaterialProxySettings);
	...
	UMaterialInstanceConstant* const BakeMaterial = UInstaLODUtilities::CreateFlattenMaterialInstance(
		FlattenMaterial, InMaterialProxySettings, SaveObjectPath, OutAssetsToSync, bIsFlipbookMaterial);

	// set static lighting flag if necessary
	static const auto AllowStaticLightingVar = IConsoleManager::Get().FindTConsoleVariableDataInt(
	...
	for (uint32 TexturePageIndex = 0u; TexturePageIndex < Material->GetTexturePageCount(); TexturePageIndex++)
	{
		...
		UTexture* const Texture = UInstaLODUtilities::ConvertInstaLODTexturePageToTexture(
			TexturePage, SaveTexturePagePath);
		...
		if (TexturePageName.Equals(TEXT("Displacement")))
		{
			...
			BakeMaterial->SetStaticSwitchParameterValueEditorOnly(FMaterialParameterInfo(TEXT("UseParallaxMapping")),
			                                                      true);
		...
		else if (TexturePageName.Equals(TEXT("NormalObjectSpace")))
		{
			...
			BakeMaterial->SetStaticSwitchParameterValueEditorOnly(
				FMaterialParameterInfo(TEXT("UseObjectSpaceNormalMap")), true);
			bStaticParametersDirty = true;
		}
...
FVector UInstaLODUtilities::CustomizePivotPosition(UStaticMesh* const StaticMesh, const FVector& PivotPosition,
                                                   const bool bLimitedRange)
{
...
void UInstaLODUtilities::GetInstaLODMeshFromMeshComponent(IInstaLOD* InstaLOD,
                                                          TSharedPtr<FInstaLODMeshComponent> MeshComponent,
                                                          InstaLOD::IInstaLODMesh* OutInstaLODMesh, int32 BaseLODIndex,
                                                          bool IsStaticMeshInWorldSpaceRequired)
{
	check(InstaLOD);
	check(OutInstaLODMesh);
	...
	{
		UInstaLODUtilities::GetInstaLODMeshFromStaticMeshComponent(InstaLOD, MeshComponent->StaticMeshComponent.Get(),
		                                                           OutInstaLODMesh, BaseLODIndex,
		                                                           IsStaticMeshInWorldSpaceRequired);
	}
	else if (MeshComponent->SkeletalMeshComponent.IsValid())
	{
		UInstaLODUtilities::GetInstaLODMeshFromSkeletalMeshComponent(
			InstaLOD, MeshComponent->SkeletalMeshComponent.Get(), OutInstaLODMesh, BaseLODIndex);
	}
	else
	{
...
void UInstaLODUtilities::RetrieveMesh(UStaticMeshComponent* const StaticMeshComponent, int32 LODIndex,
                                      FMeshDescription& OutMeshDescription, bool bPropagateVertexColours,
                                      bool bInWorldSpace)
{
	...
	FMeshMergeHelpers::ExportStaticMeshLOD(StaticMesh->GetRenderData()->LODResources[LODIndex], OutMeshDescription,
	...
	// polygon group ids to the material indices
	{
		...
		const auto fnGetStaticMaterialIndexForMaterialSlotName = [&StaticMaterials
			](const FName& MaterialSlotName) -> int32
		{
	...
	const FMeshBuildSettings& BuildSettings = bImportedMesh
		                                          ? StaticMeshModel.BuildSettings
	...
	if (BuildSettings.bRecomputeNormals)
	{
		...
		TTriangleAttributesRef<FVector3f> TriangleNormals = OutMeshDescription.TriangleAttributes().GetAttributesRef<
			FVector3f>(MeshAttribute::Triangle::Normal);
...
void UInstaLODUtilities::GetInstaLODMeshFromStaticMeshComponent(IInstaLOD* InstaLOD,
                                                                UStaticMeshComponent* StaticMeshComponent,
                                                                InstaLOD::IInstaLODMesh* OutInstaLODMesh,
                                                                int32 BaseLODIndex, bool bWorldSpace)
{
	check(InstaLOD);
	...
	NewMeshDescription.PolygonAttributes().RegisterAttribute<FVector3f>(MeshAttribute::Triangle::Normal, 1,
	                                                                    FVector3f::ZeroVector,
	                                                                    EMeshAttributeFlags::Transient);
	NewMeshDescription.PolygonAttributes().RegisterAttribute<FVector3f>(MeshAttribute::Triangle::Tangent, 1,
	                                                                    FVector3f::ZeroVector,
	                                                                    EMeshAttributeFlags::Transient);
	NewMeshDescription.PolygonAttributes().RegisterAttribute<FVector3f>(MeshAttribute::Triangle::Binormal, 1,
	                                                                    FVector3f::ZeroVector,
	                                                                    EMeshAttributeFlags::Transient);
	...
	// if necessary, recompute tangent basis
	{
		...
		TVertexInstanceAttributesRef<float> VertexInstanceBinormalSigns = MeshAttributes.
		...
		const bool bRecomputeTangents = BuildSettings.bRecomputeTangents || VertexInstanceTangents.GetNumElements() == 0
			|| VertexInstanceBinormalSigns.GetNumElements() == 0;

		if (bRecomputeNormals || bRecomputeTangents)
		{
			FStaticMeshOperations::ComputeTriangleTangentsAndNormals(NewMeshDescription, 0.0f);
			EComputeNTBsFlags NormalFlags = EComputeNTBsFlags::UseMikkTSpace |
				EComputeNTBsFlags::BlendOverlappingNormals | EComputeNTBsFlags::Tangents;
...
void UInstaLODUtilities::GetInstaLODMeshFromSkeletalMeshComponent(IInstaLOD* InstaLOD,
                                                                  USkeletalMeshComponent* SkeletalMeshComponent,
                                                                  InstaLOD::IInstaLODMesh* OutInstaLODMesh,
                                                                  int32 BaseLODIndex, UAnimSequence* const BakePose)
{
	check(InstaLOD);
	...
	InstaLOD->ConvertSkeletalLODModelToInstaLODMesh(SkeletalMeshResource->LODModels[0], OutInstaLODMesh,
	                                                BakePose != nullptr ? &BakePoseData : nullptr);
}

void UInstaLODUtilities::TransformInstaLODMesh(InstaLOD::IInstaLODMesh* InstaLODMesh, const FTransform& Transform,
                                               bool LocalToWorld)
{
	check(InstaLODMesh);
	...
	if (LocalToWorld)
	{
		...
		{
			Vertices[VertexIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.TransformPosition(InstaLODVectorHelper::InstaVecToFVector(Vertices[VertexIndex])));
		}

		for (uint64 WedgeIndex = 0; WedgeIndex < WedgeCount; WedgeIndex++)
		{
			WedgeNormals[WedgeIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.TransformVector(InstaLODVectorHelper::InstaVecToFVector(WedgeNormals[WedgeIndex])).
				          GetSafeNormal());
			WedgeBinormals[WedgeIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.TransformVector(InstaLODVectorHelper::InstaVecToFVector(WedgeBinormals[WedgeIndex])).
				          GetSafeNormal());
			WedgeTangents[WedgeIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.TransformVector(InstaLODVectorHelper::InstaVecToFVector(WedgeTangents[WedgeIndex])).
				          GetSafeNormal());
		}
	}
	else
	{
		...
		{
			Vertices[VertexIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.InverseTransformPosition(InstaLODVectorHelper::InstaVecToFVector(Vertices[VertexIndex])));
		}

		for (uint64 WedgeIndex = 0; WedgeIndex < WedgeCount; WedgeIndex++)
		{
			WedgeNormals[WedgeIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.InverseTransformVector(InstaLODVectorHelper::InstaVecToFVector(WedgeNormals[WedgeIndex])).
				          GetSafeNormal());
			WedgeBinormals[WedgeIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.InverseTransformVector(InstaLODVectorHelper::InstaVecToFVector(WedgeBinormals[WedgeIndex])).
				          GetSafeNormal());
			WedgeTangents[WedgeIndex] = InstaLODVectorHelper::FVectorToInstaVec(
				Transform.InverseTransformVector(InstaLODVectorHelper::InstaVecToFVector(WedgeTangents[WedgeIndex])).
				          GetSafeNormal());
		}
	}
...
UTexture* UInstaLODUtilities::ConvertInstaLODTexturePageToTexture(InstaLOD::IInstaLODTexturePage* InstaLODTexturePage,
                                                                  const FString& SaveObjectPath)
{
	...
	UTexture2D* const Texture = FMaterialUtilities::CreateTexture(nullptr, AssetBasePath + AssetBaseName,
	                                                              FIntPoint(InstaLODTexturePage->GetWidth(),
	                                                                        InstaLODTexturePage->GetHeight()),
	                                                              PixelData,
	                                                              TextureCompression,
...
void UInstaLODUtilities::InsertLODToMeshComponent(class IInstaLOD* InstaLOD,
                                                  TSharedPtr<FInstaLODMeshComponent> MeshComponent,
                                                  InstaLOD::IInstaLODMesh* InstaLODMesh, int32 TargetLODIndex,
                                                  UMaterialInterface* NewMaterial)
{
	// check if it's a Static Mesh or Skeletal Mesh
	if (MeshComponent->StaticMeshComponent.IsValid())
	{
		UInstaLODUtilities::InsertLODToStaticMesh(InstaLOD, MeshComponent->StaticMeshComponent->GetStaticMesh(),
		                                          InstaLODMesh, TargetLODIndex, NewMaterial);
	}
	else if (MeshComponent->SkeletalMeshComponent.IsValid())
	{
		UInstaLODUtilities::InsertLODToSkeletalMesh(
			InstaLOD, MeshComponent->SkeletalMeshComponent->GetSkeletalMeshAsset(), InstaLODMesh, TargetLODIndex,
			NewMaterial);
	}
}

void UInstaLODUtilities::InsertLODToStaticMesh(IInstaLOD* InstaLOD, UStaticMesh* StaticMesh,
                                               InstaLOD::IInstaLODMesh* InstaLODMesh, int32 TargetLODIndex,
                                               UMaterialInterface* NewMaterial)
{
	check(InstaLOD);
	...
	{
		UE_LOG(LogInstaLOD, Log, TEXT("Changing LODGroup for mesh from '%s' to 'None'."),
		       *StaticMesh->LODGroup.ToString());
...
void UInstaLODUtilities::InsertLODToSkeletalMesh(IInstaLOD* InstaLOD, USkeletalMesh* SkeletalMesh,
                                                 InstaLOD::IInstaLODMesh* InstaLODMesh, int32 TargetLODIndex,
                                                 UMaterialInterface* NewMaterial)
{
	...
	else
	{
		...
		SkeletalMesh->GetLODInfoArray()[TargetLODIndex].ScreenSize = FPerPlatformFloat(
			SkeletalMesh->GetLODInfoArray()[TargetLODIndex].ScreenSize.Default * 0.5f);
	}
...
void UInstaLODUtilities::UpdateMeshOfMeshComponent(TSharedPtr<FInstaLODMeshComponent> MeshComponent, UObject* NewMesh,
                                                   bool bRemoveOverrideMaterials)
{
	if (NewMesh == nullptr)
		return;
...
UObject* UInstaLODUtilities::SaveInstaLODMeshToDuplicateAsset(class IInstaLOD* InstaLOD,
                                                              TSharedPtr<FInstaLODMeshComponent> MeshComponent,
                                                              InstaLOD::IInstaLODMesh* InstaLODMesh,
                                                              const FString& SaveObjectPath,
                                                              bool clearMaterialAndSectionInfo)
{
...
UObject* UInstaLODUtilities::SaveInstaLODMeshToSkeletalMeshAsset(class IInstaLOD* InstaLOD,
                                                                 class InstaLOD::IInstaLODMesh* InstaLODMesh,
                                                                 const FString& SaveObjectPath)
{
	const FString AssetBaseName = FPackageName::GetShortName(SaveObjectPath);
	const FString AssetBasePath = FPackageName::GetLongPackagePath(SaveObjectPath) + TEXT("/");
	...
	USkeletalMesh* const SkeletalMesh = NewObject<USkeletalMesh>(Package, FName(*AssetBaseName),
	                                                             RF_Public | RF_Standalone);
	SkeletalMesh->InitResources();
...
UObject* UInstaLODUtilities::SaveInstaLODMeshToStaticMeshAsset(class IInstaLOD* InstaLOD,
                                                               class InstaLOD::IInstaLODMesh* InstaLODMesh,
                                                               const FString& SaveObjectPath)
{
	const FString AssetBaseName = FPackageName::GetShortName(SaveObjectPath);
...
FString UInstaLODUtilities::OpenSaveDialog(const FText& DialogTitle, const FString& DefaultPackageName,
                                           const FString& AssetNamePrefix, bool bMultiSave)
{
	...
	SaveAssetDialogConfig.ExistingAssetPolicy = bMultiSave
...
FString UInstaLODUtilities::EnsureSavePathIsUniqueForEntry(const FString& SavePath, UObject* const Entry,
                                                           TSet<FString>& PathNamesSet, const FString& Default)
{
	check(Entry);
...
TArray<InstaLODMergeData> UInstaLODUtilities::CreateMergeData(
	TArray<TSharedPtr<FInstaLODMeshComponent>>& MeshComponents, IInstaLOD* const InstaLODInterface,
	const int BaseLODIndex)
{
	...
	for (TSharedPtr<FInstaLODMeshComponent>& Component : MeshComponents)
	{
		...
		UInstaLODUtilities::GetInstaLODMeshFromMeshComponent(InstaLODInterface, Component, MergeDataEntry.InstaLODMesh,
		                                                     0, false);
...
bool UInstaLODUtilities::FinalizeScriptProcessResult(UObject* const Entry, IInstaLOD* const InstaLODInterface,
                                                     TSharedPtr<FInstaLODMeshComponent>& MeshComponent,
                                                     InstaLOD::IInstaLODMesh* const OutputMesh,
                                                     UInstaLODResultSettings* const ResultSettings,
                                                     TArray<UObject*>& OutputArray,
                                                     UMaterialInstanceConstant* const Material,
                                                     const bool bIsFreezingTransformsForMultiSelection)
{
	...
	switch (ResultSettings->SavingOption)
	{
		case EInstaLODSavingOption::InsertAsLOD:
			{
				UInstaLODUtilities::InsertLODToMeshComponent(InstaLODInterface, MeshComponent, OutputMesh,
				                                             ResultSettings->TargetLODIndex, Material);
		...
		case EInstaLODSavingOption::OnlyReturnInOutArray:
			{
				if (Entry->IsA<UStaticMesh>())
				{
					...
					if (ResultSettings->PivotPosition != EInstaLODPivotPosition::InstaLOD_Default &&
						bIsFreezingTransformsForMultiSelection)
					{
						FVector Position;
						...
						switch (ResultSettings->PivotPosition)
						{
							...
							case EInstaLODPivotPosition::InstaLOD_CustomLimited:
								Position = (ClampVector(ResultSettings->Position, FVector::ZeroVector,
								                        FVector::OneVector) - 0.5f) * 2;
								break;
				...
				else if (Entry->IsA<USkeletalMesh>())
				{
					...
					UInstaLODUtilities::InsertLODToSkeletalMesh(InstaLODInterface, SkeletalMesh, OutputMesh, 0,
					                                            Material);
